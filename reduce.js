/*
Write a function called extractValue which accepts an array of objects and a key and returns a new array with the value of each object as the key.
...
function extractValue(arr, key) {
    return arr.reduce((newArray, nextObj) => {
...
function vowelCount(str) {
    const vowels = 'aeiou';
    return str.toLowerCase().split('').reduce((vowelObj, nextLetter) => {
        if (vowels.includes(nextLetter)) {
            vowelObj[nextLetter] ? vowelObj[nextLetter]++ : vowelObj[nextLetter] = 1;
        }
        return vowelObj;
    }, {})
}
...
function partition(arr, callback) {
    return arr.reduce((twoDArray, nextElement) => {
        callback(nextElement) ? twoDArray[0].push(nextElement) : twoDArray[1].push(nextElement); 
        return twoDArray;
    }, [[], []]);
}